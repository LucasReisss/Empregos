...
public class CarrinhoContratosController implements Serializable {
	...
	public Contrato getContratos() {
		...
		List<ItemContrato> carrinhoContrato = 
				(ArrayList<ItemContrato>)Session.getInstance().getAttribute("carrinhoContrato");
	...
	public void finalizar() {
		...
		List<ItemContrato> carrinhoContrato = (ArrayList<ItemContrato>) 
				Session.getInstance().getAttribute("carrinhoContrato");
		contrato.setListaSeleFuncionario(carrinhoContrato);
		// salvar no banco
		...
		try {
			...
			Util.addMessageInfo("Contratos realizada com sucesso.");
			// limpando o carrinhoContrato
			Session.getInstance().setAttribute("carrinhoContrato", null);
		} catch (SQLException e) {
			dao.rollbackConnection();
			dao.closeConnection();
			Util.addMessageInfo("Erro ao finalizar os Contratos.");
			e.printStackTrace();
		}
